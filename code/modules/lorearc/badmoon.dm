...
/obj/structure/throne/attackby(obj/item/W as obj, mob/user as mob)
	...
	if (istype(W, /obj/item/cypher))
		...
		if(failed)
			...
			give_prize()
			msg_admin_attack("[user.name] ([user.key]) opened the throne. (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[src.x];Y=[src.y];Z=[src.z]'>JMP</a>)",ckey=key_name(user))
